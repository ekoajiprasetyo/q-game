    ...
    <div class="page-header">
        <div class="page-header-content">
            <div>
                ...
                <p class="page-subtitle">Perbarui pertanyaan bank soal</p>
            </div>
            ...
            <form action="{{ route('admin.questions.update', $question) }}" method="POST" id="questionForm" enctype="multipart/form-data">
                    ...
                    <div class="d-flex gap-3">
                        <select id="topic_select" class="form-control" style="flex: 1;">
                            ...
                            @foreach($topics as $topic)
                                <option value="{{ $topic->id }}" {{ $question->material?->topic_id == $topic->id ? 'selected' : '' }}>
                        ...
                        <select id="material_id" name="material_id" class="form-control" style="flex: 1;" required>
                            ...
                            @foreach($materials as $material)
                                <option value="{{ $material->id }}" data-topic="{{ $material->topic_id }}" {{ old('material_id', $question->material_id) == $material->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <div class="type-selector">
                        <label class="type-option {{ old('question_type', $question->question_type) === 'multiple_choice' ? 'selected' : '' }}" data-type="multiple_choice">
                            <input type="radio" name="question_type" value="multiple_choice" {{ old('question_type', $question->question_type) === 'multiple_choice' ? 'checked' : '' }} class="hidden-radio">
                            <div class="type-option-icon">🔤</div>
                            <div class="type-option-label">Pilihan Ganda</div>
                        </label>
                        <label class="type-option {{ old('question_type', $question->question_type) === 'multiple_answer' ? 'selected' : '' }}" data-type="multiple_answer">
                            <input type="radio" name="question_type" value="multiple_answer" {{ old('question_type', $question->question_type) === 'multiple_answer' ? 'checked' : '' }} class="hidden-radio">
                            <div class="type-option-icon">☑️</div>
                            <div class="type-option-label">PG Kompleks</div>
                        </label>
                        <label class="type-option {{ old('question_type', $question->question_type) === 'true_false' ? 'selected' : '' }}" data-type="true_false">
                            <input type="radio" name="question_type" value="true_false" {{ old('question_type', $question->question_type) === 'true_false' ? 'checked' : '' }} class="hidden-radio">
                            <div class="type-option-icon">✓✗</div>
                            <div class="type-option-label">Benar / Salah</div>
                        </label>
                        <label class="type-option {{ old('question_type', $question->question_type) === 'short_answer' ? 'selected' : '' }}" data-type="short_answer">
                            <input type="radio" name="question_type" value="short_answer" {{ old('question_type', $question->question_type) === 'short_answer' ? 'checked' : '' }} class="hidden-radio">
                            <div class="type-option-icon">⌨️</div>
                ...
                <div class="form-group">
                    ...
                    <div class="image-upload-section {{ $question->image_url ? 'has-image' : '' }}" id="imageUploadSection">
                        <div id="imageUploadPlaceholder" style="{{ $question->image_url ? 'display: none;' : '' }}">
                        ...
                        <input type="hidden" id="imageScale" name="image_scale" value="{{ $question->image_scale ?? 100 }}">
                ...
                <input type="hidden" name="correct_answer" id="correctAnswer" value="{{ old('correct_answer', $question->correct_answer) }}">
                ...
                <div class="form-group">
                    ...
                    <div class="settings-grid" style="grid-template-columns: repeat(2, 1fr);">
                        ...
                        <div>
                            ...
                            <input type="number" id="points" name="points" class="form-control" value="{{ old('points', $question->points) }}" min="1" max="100" required>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    let correctAnswer = '{{ old('correct_answer', $question->correct_answer) }}';
    ...
    @if($question->question_type === 'multiple_answer' && isset($question->options['choices']))
        multiAnswerOptions = @json($question->options['choices']);
        const existingCorrectAnswers = @json($question->options['correct_answers'] ?? []);
        multiAnswerOptions = multiAnswerOptions.map(opt => ({
            ...opt,
            isCorrect: existingCorrectAnswers.includes(opt.key)
        }));
    @else
        multiAnswerOptions = [
    ...
    function handleQuestionTypeChange(type) {
        ...
        if (type === 'multiple_choice') {
            ...
            if (correctAnswer !== 'T' && correctAnswer !== 'F') {
                correctAnswerInput.value = 'T';
            }
            trueFalseOptions.forEach(opt => opt.classList.toggle('selected', opt.dataset.value === correctAnswerInput.value));
    ...
    let currentImageData = document.getElementById('imagePreview').src || null;
    let currentScale = parseInt(document.getElementById('imageScale').value) || 100;

    // Apply saved scale to existing image on page load
    (function() {
        const preview = document.getElementById('imagePreview');
        if (preview.src && currentScale !== 100) {
